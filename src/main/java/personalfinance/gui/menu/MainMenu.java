...
public class MainMenu extends JMenuBar implements Refresh, EnableEditDelete { //класс, для создания меню программы
    ...
    private void init() {
        ...
        addMenuItem(file, Text.get("MENU_FILE_NEW"), Style.ICON_MENU_FILE_NEW, HandlerCode.MENU_FILE_NEW, KeyEvent.VK_N);//KeyEvent.VK_N - реализация горячей клавиши (ctrl + N)
    ...
    private JMenuItem addMenuItem(JMenu menu, String title, ImageIcon icon, String action, int key) { //вспомогательный метод, помогающий уменьшить кол-во кода для пунктов, где есть горячие клавиши
    ...
    private JMenuItem addMenuItem(JMenu menu, String title, ImageIcon icon, String action) { //если у раздела нет горячей клавиши
