...
public class Filter { //класс для реализации фильтра по дате
    ...
    public void next() { // шаг следующий ">"
       offset(1);
    }

    public void prev() { //шаг предыдущий "<"
       offset(-1);
    }

    public void nextPeriod() { //значение шага(день/месяц/год)
        step += 1;
        if (step > STEP_YEAR)
            step = STEP_DAY;
    ...
    public boolean check (Date date) { //проверка
    ...
    private void offset(int i) { //сдвиг календаря на шаг
